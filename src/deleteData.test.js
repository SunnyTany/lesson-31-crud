...
//   it('returns error message when the fetch call fails', async () => {
//     // Симулюємо помилку fetch запиту
//     fetch.mockRejectedValue(new Error(`Failed to delete post with id ${id}. Status: ${response.status}`))

//     const id = 3 // Використовуємо ідентифікатор, який симулює сценарій помилки
//     const result = await deleteData(id)
//     expect(result).toBe(`Failed to delete post with id ${id}. Status: ${response.status}`) // Перевіряємо, що функція повернула повідомлення помилки
